...
public class SharingRenturn extends JPanel{
	JTable table;
	JScrollPane tablePane;
	JButton returnbtn;
	String curUser;
	String title[]= {"编号","名称","类别","状态","发布人"};
	String[][]data=GoodDao.returnlist(curUser);
	public SharingRenturn(String unname) {
		curUser=unname;
		data=GoodDao.returnlist(curUser);
		//创建表格并展示数据
	...
	private void addListeners() {
		...
		returnbtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//更新表格数据
				int c=JOptionPane.showConfirmDialog(SharingRenturn.this, "确认要归还该物品吗","确认对话框",JOptionPane.YES_NO_OPTION);
				if(c==JOptionPane.NO_OPTION) {
					...
					if(row==-1) {
						JOptionPane.showMessageDialog(SharingRenturn.this, "请先选择你要归还的物品!!!");
						return ;
					}
					String id=data[row][0];
					if(!id.equals("")) {
						String info=GoodDao.returngood(id);
						if(info.equals("归还成功")) {
							JOptionPane.showMessageDialog(SharingRenturn.this, "归还成功");
						}
						else {
							JOptionPane.showMessageDialog(SharingRenturn.this, info);
						}
					}
